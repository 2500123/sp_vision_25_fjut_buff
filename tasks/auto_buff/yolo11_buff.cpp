...
std::vector<YOLO11_BUFF::Object> YOLO11_BUFF::get_multicandidateboxes(cv::Mat & image)
{
  ...
  float pad_x = 0.0f;
  float pad_y = 0.0f;
  const float factor = fill_tensor_data_image(input_tensor, image, &pad_x, &pad_y);
  ...
  // 输出格式: 每列代表一个候选框. 前4行为[cx, cy, ow, oh], 第5行为score, 后续为NUM_POINTS*2的关键点[x, y].
  for (int i = 0; i < det_output.cols; ++i) {
    ...
    if (score > ConfidenceThreshold) {
      ...
      cv::Rect box;
      // Compensate letterbox padding offsets (pad_x/pad_y are in input-tensor pixel coordinates)
      box.x = static_cast<int>((cx - 0.5f * ow - pad_x) * factor);
      ...
      for (int j = 0; j < NUM_POINTS; ++j) {
        const float x = (kpts.at<float>(j * 2 + 0, 0) - pad_x) * factor;
        const float y = (kpts.at<float>(j * 2 + 1, 0) - pad_y) * factor;
...
std::vector<YOLO11_BUFF::Object> YOLO11_BUFF::get_onecandidatebox(cv::Mat & image)
{
  ...
  float pad_x = 0.0f;
  float pad_y = 0.0f;
  const float factor = fill_tensor_data_image(input_tensor, image, &pad_x, &pad_y);  // 填充图片到合适的input size
  ...
  if (max_confidence > ConfidenceThreshold) {
    ...
    obj.rect.x = static_cast<int>((cx - 0.5f * ow - pad_x) * factor);
    ...
    for (int i = 0; i < NUM_POINTS; ++i) {
      const float x = (kpts.at<float>(i * 2 + 0, 0) - pad_x) * factor;
      const float y = (kpts.at<float>(i * 2 + 1, 0) - pad_y) * factor;
...
float YOLO11_BUFF::fill_tensor_data_image(
  ov::Tensor & input_tensor, const cv::Mat & input_image, float * pad_x, float * pad_y) const
{
  ...
  // Current implementation places the resized image at the top-left, so padding offset is (0, 0).
  // Exposing these makes postprocessing robust if the letterbox placement changes in the future.
  if (pad_x) *pad_x = 0.0f;
  if (pad_y) *pad_y = 0.0f;
  cv::Mat blob_image;
